...
class Giveaway extends EventEmitter {
  ...
  get remainingTimeText() {
    ...
    const dayUnit =
      ...
      minuteUnit =
        minutes < 2 &&
        (this.messages.units.pluralS ||
          this.messages.units.minutes.endsWith("s"))
      ...
      secondUnit =
        seconds < 2 &&
        (this.messages.units.pluralS ||
          this.messages.units.seconds.endsWith("s"))
  ...
  edit(options = {}) {
    return new Promise(async (resolve, reject) => {
      ...
      if (!this.channel) {
        return reject(
          "Unable to get the channel of the giveaway with message ID " +
            this.messageID +
            "."
        );
      }
   ...
   */
    ...
    return new Promise(async (resolve, reject) => {
      ...
      if (winners.length > 0) {
        ...
        await this.message
          .edit(this.messages.giveawayEnded, { embed })
          .catch(() => {});
        ...
        if (messageString.length <= 2000)
          this.message.channel.send({ content: messageString });
        else {
