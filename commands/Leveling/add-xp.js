...
module.exports = {
  ...
  run: async (client, message, args) => {
    ...
    if (!member)
      return message.channel.send({content:
        `You specified incorrect usage\nCorrect usage: add-xp <member> <amount>`
      }
      );
    const xp = args.slice(1).join(" ");
    if (!xp || !Number(xp)) return message.channel.send({content: `Xp must be a number!`});
    ...
    message.channel.send({embeds: [
      new MessageEmbed()
        ...
        .setColor("RANDOM")
        .setFooter(
          `Added ${xp} XP to ${member.user.tag} || Requested by: ${message.author.tag}`
        )
    ]
    }
    );
