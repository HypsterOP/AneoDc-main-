...
module.exports = {
  ...
  run:async (client , message , args, quick) => {
    ...
    let author = quick.fetch(`money_${user.id}`)
    ...
    if (args[0] == 'car') {
        ...
        quick.fetch(`car_${user.id}`);
        quick.set(`car_${user.id}`, true)

      
        quick.subtract(`money_${user.id}`, 5000)
        ...
        quick.fetch(`miner_${user.id}`);
        quick.set(`miner_${user.id}`, true)

      
        quick.subtract(`money_${user.id}`, 1000)
        ...
        quick.fetch(`house_${user.id}`);
        quick.set(`house_${user.id}`, true)

      
        quick.subtract(`money_${user.id}`, 20000)
        ...
        quick.fetch(`mc_${user.id}`);
        quick.set(`mc_${user.id}`, true)

      
        quick.subtract(`money_${user.id}`, 50)
        message.inlineReply(`Succesfully bought **Minecraft** for **50** Coins!`)
            ...
            quick.fetch(`diamond_${user.id}`)
            quick.set(`diamond_${user.id}`, true)

            quick.subtract(`money_${user.id}`,250000000)
