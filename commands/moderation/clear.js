module.exports = {
    ...
    run : async(client, message, args) => {
        ...
        if(member) {
            ...
            if(!message.member.hasPermission('MANAGE_MESSAGES')) return message.channel.send('Hello there, i see you do not have the permission to manage messages <a:warning:818327691052318731>')
            if(!args[0]) return message.channel.send('Please give me a number of messages to delete/ purge.')
            if(isNaN(args[0])) return message.channel.send('Only number are allowed! <a:warning:818327691052318731>')
